...
# accompanied by a formatter in `basketball_data_emporium.formatters` (TBD).
...
ALL_COLUMN_CONTRACTS: list[ColumnContract] = [
    ...
    ColumnContract(
        key="full_name",
        # Endpoint SQL reads this through the schema compatibility layer
        # as `display_name`; lineage stays pointed at the physical column.
        label="Full name",
    ...
    # Endpoint SQL reads `full_name` through the schema compatibility
    # layer. The manifest still declares the physical lineage columns.
    # -----------------------------------------------------------------
    ColumnContract(
        key="team_id",
        label="Team ID",
