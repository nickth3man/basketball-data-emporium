...
`basketball_data_emporium.server.app`) wraps any of these into the FastAPI
...
class BasketballDataEmporiumError(Exception):
...
class InvalidSearchError(BasketballDataEmporiumError):
    code = INVALID_SEARCH
    status = 400


class BadRequestError(BasketballDataEmporiumError):
    code = BAD_REQUEST
    status = 400


class InvalidPlayerError(BasketballDataEmporiumError):
    code = INVALID_PLAYER
    status = 404


class InvalidTeamError(BasketballDataEmporiumError):
    code = INVALID_TEAM
    status = 404


class InvalidSeasonError(BasketballDataEmporiumError):
    code = INVALID_SEASON
    status = 404


class RateLimitJailedError(BasketballDataEmporiumError):
...
class SchemaDriftError(BasketballDataEmporiumError):
    code = SCHEMA_DRIFT
    status = 500


class InternalError(BasketballDataEmporiumError):
    code = INTERNAL_ERROR
    status = 500
